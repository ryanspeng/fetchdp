...
def get_ids(soup):
    ...
    for div in soup.find_all('div', class_='shop-list'):
        return num, [href.get('href')[6:] for href in div.find_all('a') if href.get('href', '').startswith('/shop/')]
    return num, []
...
def get_category_shop(category, region_id, page):
    if page >= 51:
        region_id += 1
        page = 1
    if region_id >= len(regions):
        return ''
    url = "http://www.dianping.com/search/category/2/%s/%s%s/p%s" % (categorys[category], category, regions[region_id], page)
...
if __name__ == '__main__':
    ...
    sids = load_shop_ids()
    while True:
        url = get_category_shop(category, region_id, page)
        if url:
